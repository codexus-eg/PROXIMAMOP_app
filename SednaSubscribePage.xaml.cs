...
public partial class SednaSubscribePage : ContentPage
{
    private const string BaseUrl = "http://195.3.223.75:5003";
    private const string UserIdPreferenceKey = "sedna_user_id";
    ...
    private async void OnSubmitClicked(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            var response = await _httpClient.PostAsync($"{BaseUrl}/api/sedna/submit", form);
    ...
    private async Task RefreshStatusAsync()
    {
        try
        {
            ...
            var response = await _httpClient.GetAsync($"{BaseUrl}/api/sedna/status/{_userId}");
